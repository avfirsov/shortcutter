...
var keydown$ = rxjs_1.fromEvent(document, 'keydown');
var keyup$ = rxjs_1.fromEvent(document, 'keyup');

var filterKey = function filterKey(observable) {
  return function (key) {
    return observable.pipe(operators_1.filter(function (e) {
      return e.key.toLowerCase() === key.toLowerCase();
    }), operators_1.tap(function (e) {
      return defaultOptions.preventDefault && e.preventDefault();
    }));
  };
};

var _a = [keydown$, keyup$].map(filterKey),
    keyDownByKey = _a[0],
    keyUpByKey = _a[1];
...
/** @class */
function () {
  function Shortcutter(options) {
    this._shortcuts = [];
    this.options = __assign({}, defaultOptions);
    this.options = __assign(__assign({}, this.options), options);
  }

  Shortcutter.prototype.on = function (keys, cb) {
    var _this = this;

    var subscrition = rxjs_1.combineLatest(keys.map(function (key) {
      return rxjs_1.merge(keyDownByKey(key).pipe(operators_1.mapTo(true)), keyUpByKey(key).pipe(operators_1.mapTo(false)));
    })).pipe(operators_1.pairwise(), //предотвращаем залипание
    operators_1.filter(function (_a) {
      var prevState = _a[0],
          curState = _a[1];
      return !_this.options.preventSeries || prevState.some(function (p, i) {
        return p !== curState[i];
      });
      ...
      var _ = _a[0],
          curState = _a[1];
      return curState;
    }), operators_1.filter(function (keysState) {
      return keysState.every(Boolean);
    })).subscribe(cb);
    ...
    return function () {
      var ind = _this._shortcuts.findIndex(function (_a) {
        var cSub = _a.subscrition;
        return cSub === subscrition;
      });

      _this._shortcuts = __spreadArrays(_this._shortcuts.slice(0, ind), _this._shortcuts.slice(ind + 1, _this._shortcuts.length));
...
exports.Shortcutter = Shortcutter;
},{"rxjs":"node_modules/rxjs/_esm5/index.js","rxjs/operators":"node_modules/rxjs/_esm5/operators/index.js"}],"index.ts":[function(require,module,exports) {
"use strict";
...
var shortcutter_1 = require("./shortcutter");

var shortcutter = new shortcutter_1.Shortcutter({
  preventDefault: true,
  preventSeries: false
});
var cancelCtrlAltD = shortcutter.on(['Control', 'Alt', 'd'], function () {
  console.log('pressed', 'Control', 'Alt', 'd');
});
var cancelCtrlShftL = shortcutter.on(['Control', 'Shift', 'l'], function () {
  console.log('pressed', 'Control', 'Shift', 'l');
});
setTimeout(cancelCtrlAltD, 4000);
setTimeout(cancelCtrlShftL, 8000);
},{"./shortcutter":"shortcutter.ts"}],"../../../AppData/Roaming/npm/node_modules/parcel-bundler/src/builtins/hmr-runtime.js":[function(require,module,exports) {
...
//# sourceMappingURL=/shortcuts.77de5100.js.map